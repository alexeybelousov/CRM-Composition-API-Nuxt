...
export const actions = {
  ...
  async fetchCategoryById({ commit, rootGetters }, id) {
    try {
      const uid = rootGetters["auth/user"] && rootGetters["auth/user"].uid
      const category =
        (
  ...
  async createCategory({ commit, rootGetters }, { title, limit }) {
    try {
      const uid = rootGetters["auth/user"] && rootGetters["auth/user"].uid
  ...
  async updateCategory({ commit, rootGetters }, { title, limit, id }) {
    try {
      const uid = rootGetters["auth/user"] && rootGetters["auth/user"].uid
      await fb
        .database()
