<template>
  <div>
    ...
    <div v-if="!loading" class="history-chart">
      <Chart :chart-data="chartData" :width="300" :height="300" />
    </div>
  ...
  setup(props, ctx) {
    ...
    onMounted(async () => {
      ...
      filteredRecords.value = records.value.map(record => ({
        ...record,
        categoryName: categories.value.find(cat => cat.id === record.categoryId)
          .title,
      ...
      chartData.datasets = [
        {
          ...
          data: categories.value.map(cat =>
            records.value.reduce((total, rec) => {
